...
public partial class HeadquartersIndex
{
    ...
    private MudTable<Headquarter> table = new();
    private readonly int[] pageSizeOptions = { 10, 25, 50, int.MaxValue };
    private int totalRecords = 0;
    private bool loading;
    private const string baseUrl = "api/Headquarters";
    ...
    private async Task<TableData<Headquarter>> LoadListAsync(TableState state, CancellationToken cancellationToken)
    {
        ...
        var responseHttp = await Repository.GetAsync<List<Headquarter>>(url);
        if (responseHttp.Error)
        {
            var message = await responseHttp.GetErrorMessageAsync();
            Snackbar.Add(Localizer[message!], Severity.Error);
            return new TableData<Headquarter> { Items = [], TotalItems = 0 };
        }
        if (responseHttp.Response == null)
        {
            return new TableData<Headquarter> { Items = [], TotalItems = 0 };
        }
        return new TableData<Headquarter>
        {
    ...
    private async Task ShowModalAsync(int id = 0, bool isEdit = false)
    {
        ...
        if (isEdit)
        {
            ...
            dialog = DialogService.Show<HeadquarterEdit>($"{Localizer["Edit"]} {Localizer["Headquarter"]}", parameters, options);
        }
        else
        {
            dialog = DialogService.Show<HeadquarterCreate>($"{Localizer["New"]} {Localizer["Headquarter"]}", options);
        }
