...
public partial class TicketsIndex
{
    private List<Ticket>? Tickets { get; set; }
    private MudTable<Ticket> table = new();
    ...
    private async Task<TableData<Ticket>> LoadListAsync(TableState state, CancellationToken cancellationToken)
    {
        ...
        var responseHttp = await Repository.GetAsync<List<Ticket>>(url);
        if (responseHttp.Error)
        {
            var message = await responseHttp.GetErrorMessageAsync();
            Snackbar.Add(Localizer[message!], Severity.Error);
            return new TableData<Ticket> { Items = [], TotalItems = 0 };
        }
        if (responseHttp.Response == null)
        {
            return new TableData<Ticket> { Items = [], TotalItems = 0 };
        }
        return new TableData<Ticket>
        {
